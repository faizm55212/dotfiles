...
deck(Monitor *m) {
	...
	for(i = 0, my = gappih, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if(i < m->nmaster) {
			h = (m->wh - my) / (MIN(n, m->nmaster) - i) - gappih;
			resize(c, m->wx + gappih, m->wy + my, mw - (2*c->bw) - gappih*(5-ns)/2, h - (2*c->bw), False);
			my += HEIGHT(c) + gappih;
		}
		else
			resize(c, m->wx + mw + gappih/ns, m->wy + gappih, m->ww - mw - (2*c->bw) - gappih*(5-ns)/2, m->wh - (2*c->bw) - 2*gappih, False);
}
...
tile(Monitor *m)
{
	unsigned int i, n, h, smh, r, oe = enablegaps, ie = enablegaps, mw, my, ty;
	Client *c;
	...
	for (i = 0, my = ty = m->gappoh*oe, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			if (my + HEIGHT(c) + m->gappih*ie < m->wh)
				my += HEIGHT(c) + m->gappih*ie; 			//MIGHT HAVE ERROR
		} else {
			smh = m->mh * m->smfact;
			if(!(nexttiled(c->next))){
				r = n - i;
			...
			if(h < minwsz) {
				...
				resize(c, m->wx + mw + m->gappov*oe, m->wy + ty, m->ww - mw - (2*c->bw) - 2*m->gappov*oe, h - (2*c->bw), False);
			if(!(nexttiled(c->next)))
				ty += HEIGHT(c) + smh;
			else
